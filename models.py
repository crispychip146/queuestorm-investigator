...
class TransactionHistoryEntry(BaseModel):
    ...
    @classmethod
    def normalize_tx_enums(cls, v: Any) -> Any:
        if isinstance(v, str):
...
class AnalyzeTicketRequest(BaseModel):
    ...
    # Reserved for future extension; logged securely but not forwarded to LLM to prevent prompt injection.
    metadata: Optional[Dict[str, Any]] = None

    @field_validator("language", "channel", "user_type", mode="before")
    @classmethod
    def normalize_req_enums(cls, v: Any) -> Any:
        if isinstance(v, str):
            return v.lower().replace("-", "_").replace(" ", "_")
        return v
...
class AnalyzeTicketResponse(BaseModel):
    ...
    @field_validator("evidence_verdict", "case_type", "severity", "department", mode="before")
    @classmethod
    def normalize_resp_enums(cls, v: Any) -> Any:
        if isinstance(v, str):
            return v.lower().replace("-", "_").replace(" ", "_")
        return v
